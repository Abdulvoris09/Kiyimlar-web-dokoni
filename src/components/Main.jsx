...
const Main = ({ addCart, addLike, removeLike, e }) => {
  ...
  return (
    ...
    >
      {/* img  */}
      <div>
        <img style={{ objectFit: 'cover', objectPosition: 'top' }} className="w-full h-64 md:h-72 rounded-t-lg brightness-90 duration-300" src={e.img} alt="new product" />
        {/* like  */}
        ...
        <div className="flex items-center p-2.5 pt-0">
          ...
          <div>
            <Link to={`/erkaklerkiyimlari/${e.id}`} className="px-5 py-2 font-semibold text-base leading-6  text-white bg-[#1B37A3] rounded-md">
              {/* <img className='w-6 h-6' src="./public/img/i.png" alt="haqida" /> */}
              Batafsil
            </Link>
          </div>
        </div>
